...
module.exports = app => {
  ...
  // add isUnique validator
  Sequelize.prototype.validateIsUnique = function(col, msg) {
    const conditions = { where: {} };
    msg = (!msg) ? col + ' must be unique' : msg;
    return function(value, next) {
      const self = this;
      conditions.where[col] = value;
      this.constructor.findOne(conditions).then(function(object) {
        return object ? next(msg) : next();
      }).catch(next);
    };
  };
