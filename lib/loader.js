...
module.exports = app => {
  ...
  class ContextSequelize {
    ...
    query(sql, options) {
      ...
      return this.ctx.app.model.query(sql, options);
    }
  }
  app.ContextSequelize = ContextSequelize;
...
function loadModel(app) {
  ...
  for (const name of Object.keys(app[MODELS])) {
    ...
    if ('sequelize' in klass) {
      ...
      defineConextModel(app.ContextSequelize, name, klass);
...
function defineConextModel(ContextSequelize, name, klass) {
  Object.defineProperty(ContextSequelize.prototype, name, {
    get() {
      // this is instance of ContextModelClass
      return klass.contextify(this.ctx);
    },
  });
}
